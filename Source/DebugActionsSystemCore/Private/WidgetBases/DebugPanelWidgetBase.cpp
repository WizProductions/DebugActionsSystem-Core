...
bool UDebugPanelWidgetBase::AddDebugActionChildWidget_Implementation(
	int ChildDebugActionIndex,
	int DepthLevel,
	UDebugActionBase* ChildDebugAction,
	UDebugActionBase* ParentDebugAction
)
{
...
bool UDebugPanelWidgetBase::CreateDebugActionWidget_Implementation(
	UDebugActionBase* LinkedDebugAction,
	class UDebugActionWidgetBase*& NewWidget,
...
)
{
	...
	FName UniqueWidgetName = MakeUniqueObjectName(WidgetTree, DAWClass->GetClass(),
	FName(FString::Printf(TEXT("DebugActionWidget_%s"), *LinkedDebugAction->GetDebugActionTitle().ToString())));
	NewWidget = WidgetTree->ConstructWidget<UDebugActionWidgetBase>(DAWClass, UniqueWidgetName);
...
void UDebugPanelWidgetBase::OnFolderStateChange_Implementation( bool bIsDeveloped, bool bIsNewFolderClicked, UDebugActionBase* InDebugActionFolder )
{
	...
	if (bIsNewFolderClicked == false) {
		SetActiveDebugInputSlotsVisibility(bIsDeveloped
...
{
	if (DebugActionsDepthsArray.Num() < Depth || Depth < 0)
...
bool UDebugPanelWidgetBase::UnassignSlotToDebugInput( class UDebugInputBase* InDebugInput )
{
	...
	//Not assigned
	if (InDebugInput->GetDebugInputSlotWidget() == NULL)
		return true;

	InDebugInput->GetDebugInputSlotWidget()->RemoveInputWidget();
	return true;
}
...
void UDebugPanelWidgetBase::Internal_FindAndInitChildDebugActions(
	UDebugSubsystem* Subsystem,
